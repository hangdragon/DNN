...
class LogicGate:
    def __init__(self,name='',train_x_=[],y=[],w_=[],lr=0.5): #멤버 변수 : 논리게이트 이름,트레이닝 x들,라벨 y들,시스템 벡테 w_,learning rate인 lr
        ...
        ###################시스템 벡터 초기화####################
        print('\n시스템 벡터를 가우사안 분포를 택하여 랜덤하게 초기화시키려 합니다. 원하시는 평균과 표준분포를 입력하세요.')
        mean = float(input('평균 :').strip())
        vari = float(input('표준편차 :').strip())
        self.w_ = np.random.normal(mean,vari,len(tmp_x[0])).reshape(self.k,-1)
...
and_gate = LogicGate()
and_gate.slp_network()
and_gate.test_and_plot()