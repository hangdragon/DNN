...
class SLP_ki :
    ...
    def __init__(self): #생성자는 아키텍처를 선택하고, 웨이트들을 이니셜라이즈 해준다.
        ...
        self.i = int(input('원하시는 출력의 갯수를 입력하세요 : ')) #ex) (0,0)과 (0,1)은 (1,2)로 , (1,0)과 (1,1)은 (3,4)로 가게 하고싶다면 출력벡터가 길이가 2인 1차원 벡터이므로 n = 2
    ...
    def gradient_back_propagation(self,loop_number = 40000):
            ...
            for i in range(self.i):
                for k in range((self.k)+1):
                    ...
                    for n in range(self.n) :
                        delta_i_[n][i] = (self.y_[n][i] - self.y_est[n][i]) * self.y_est[n][i] * (1 - self.y_est[n][i])
                        gradient += -(self.x_[n][k])*delta_i_[n][i]
                    #n번 루프 다 돌았고,
